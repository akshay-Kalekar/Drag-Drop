...
//Select the element with area draggable
function drag(event){
    event.dataTransfer.setData("text",event.target.id); 

    document.getElementById("status").innerHTML=`Drag`
}
// Reset the container 2 to their initial state
function resetContainers() {
    ...
    //Reset the container 1 to their initial state

    bg1Order.forEach(item => bg1.appendChild(item));
    ...
    text1Order.forEach(item => text1.appendChild(item));
